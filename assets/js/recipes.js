document.addEventListener("DOMContentLoaded", function() {
    ...
    orderData(0); // Initialize 
    order = shuffleArray(order); // Random shuffle
    orderData(-1);
    selectRecipe(selected);
    ...
    function orderData(column) {
        ...
        fillTable();
        populateGrid(filteredOrder);
    }
    
    function populateGrid(gridOrder) {
        gridRecipes.innerHTML = '';
        for (let j = 0; j < gridOrder.length; j++) {
            const i = gridOrder[j];
            const gridRecipe = document.createElement("a");
            ...
            recipeTitle.textContent = data[i]['name'];
            gridRecipe.appendChild(recipeTitle);
            
            gridRecipes.appendChild(gridRecipe);
        }
    }
